...
CREATE TABLE IF NOT EXISTS `BurgerCity`.`Products` (
  ...
  CONSTRAINT `fk_Products_CategoryHelper1`
    FOREIGN KEY (`productsId`)
    REFERENCES `BurgerCity`.`CategoryHelper` (`productsId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `BurgerCity`.`Category` (
  ...
  CONSTRAINT `fk_Category_CategoryHelper1`
    FOREIGN KEY (`categoryId`)
    REFERENCES `BurgerCity`.`CategoryHelper` (`categoryId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION);
...
CREATE TABLE IF NOT EXISTS `BurgerCity`.`AdressHelper` (
  ...
  CONSTRAINT `fk_AdressHelper_Account1`
    FOREIGN KEY (`addressId`)
    REFERENCES `BurgerCity`.`Account` (`adressId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION);
